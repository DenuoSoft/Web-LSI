...
export default function Logo({ $isHomePage = true, ...props }: LogoProps) {
  const [isHovered, setIsHovered] = useState(false);
  
  // Определяем цвета в зависимости от страницы
  const mainColor = $isHomePage ? '#c8d2e6' : '#555A69';
  const accentColor = $isHomePage ? '#c8d2e6' : '#8237FF';
  const hoverColor = '#8237FF';
  ...
  return (
    ...
    >
      ...
      <polygon
        ...
        style={{ 
          fill: isHovered ? hoverColor : ($isHomePage ? '#c8d2e6' : accentColor),
          transition: 'fill 0.3s ease'
        }}
      />
    </svg>
