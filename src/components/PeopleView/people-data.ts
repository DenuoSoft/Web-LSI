import type {peopleProps, PersonJsonData} from '../../models/people-model';
...
import scottcv from '../../data/cv/Scott.json';
import lamzincv from '../../data/cv/Lamzin.json';
import panovcv from '../../data/cv/Panov.json';
import naberezhnayacv from '../../data/cv/Naberezhnaya.json';
import altermancv from '../../data/cv/Alterman.json';
import boikovcv from '../../data/cv/Boikov.json';
import klushincv from '../../data/cv/Klushin.json';
import shabalincv from '../../data/cv/Shabalin.json';


const asPersonJsonData = (data: unknown): PersonJsonData => {
	return data as PersonJsonData;
};

const jsonDataMap: Record<number, PersonJsonData> = {
	1: asPersonJsonData(sosedkincv),
	2: asPersonJsonData(scottcv),
	3: asPersonJsonData(lamzincv),
	4: asPersonJsonData(panovcv),
	5: asPersonJsonData(naberezhnayacv),
	6: asPersonJsonData(altermancv),
	7: asPersonJsonData(boikovcv),
	8: asPersonJsonData(klushincv),
	9: asPersonJsonData(shabalincv)
...
export const people: peopleProps[] = [
	{
		...
		email: "denis.sosedkin@denuo.legal",
		jsonData: jsonDataMap[1]
	},
	{
		id: 2,
		title: 'Scott Antel',
		...
		about: '',
		position: 'Partner',
		email: "scott.antel@denuo.legal",
		jsonData: jsonDataMap[2]
	},
	{
		id: 3,
		title: 'Philip Lamzin',
		...
		position: 'Partner',
		email: "philip.lamzin@denuo.legal",
		jsonData: jsonDataMap[3]
	},
	{
		id: 4,
		title: 'Andrey Panov',
		img: panov,
		badge: ['Litigation & Arbitration'],
		about:
		...
		position: 'Partner',
		email: "andrey.panov@denuo.legal",
		jsonData: jsonDataMap[4]
	},
	{
		id: 5,
		...
		email: "julia.naberezhnaya@denuo.legal",
		jsonData: jsonDataMap[5],
	...
	},
	{
		...
		img: alterman,
		badge: ['Corporate', 'Regulatory & Compliance'],
...
export const getPersonById = (id: number): peopleProps | undefined => {
	return people.find(person => person.id === id);
};

// Функция для получения JSON данных по ID
export const getPersonJsonData = (id: number): PersonJsonData | null => {
	return jsonDataMap[id] || null;
};